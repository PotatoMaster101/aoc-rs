...
impl<T: Copy + Num> Area<T> {
    ...
    /// Returns whether a `Pos<T>` is at the boundary of this `Area<T>`.
    #[inline]
    pub fn is_bound(&self, p: &Pos<T>) -> bool {
...
mod test {
    ...
    fn test_is_bound() {
        let sut = Area { max_x: 10, max_y: 10, min_x: 0, min_y: 0 };
        assert!(sut.is_bound(&Pos { x: 10, y: 10 }));
        assert!(sut.is_bound(&Pos { x: 3, y: 0 }));
        assert!(sut.is_bound(&Pos { x: 0, y: 4 }));
        assert!(sut.is_bound(&Pos { x: 0, y: 0 }));
        assert!(!sut.is_bound(&Pos { x: 1, y: 1 }));
        assert!(!sut.is_bound(&Pos { x: 9, y: 9 }));
    }
